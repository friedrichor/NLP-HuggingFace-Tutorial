...
def process_data(data, mode: str, num: int):
    ...
    print(f"{num} data were selected from the {mode} set")
    ...
    dataset_folder = os.path.join(sys.path[0], 'dataset_partial')
...
def process_test_data_for_evaluate(data, num: int):
    dialogue_dic_list = []
    for i, per_data in tqdm(enumerate(data)):
        if i == num:
            break
...
if __name__ == '__main__':
    ...
    process_data(train_data, 'train', 4000)

    validation_data = dataset['validation']
    process_data(validation_data, 'validation', 500)

    test_data = dataset['test']
    process_data(test_data, 'test', 500)
    process_test_data_for_evaluate(test_data, 500)
